    ...
    <AnimatePresence>
      ...
      <motion.div
        ...
        animate={mergeElements ? {
          opacity: 1,
          scale: [1, 1.2, 0],
          x: typeof window !== 'undefined' ? -(window.innerWidth * 0.85 - window.innerWidth / 2) + 80 : 0,
          y: typeof window !== 'undefined' ? -(window.innerHeight * 0.8 - window.innerHeight / 2) + 40 : 0,
          rotate: -180
        } : { opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0 }}
        transition={{ duration: mergeElements ? 1.5 : 0.8, delay: 0.8 }}
      >
