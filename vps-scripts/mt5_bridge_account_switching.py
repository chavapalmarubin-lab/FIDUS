...
def load_account_credentials():
    """Load account credentials from MongoDB"""
...
def get_account_data_after_login():
    """Get account data after successful login"""
    try:
...
def refresh_all_accounts():
    """
    Loop through ALL accounts, login to each, and get their data
    This is the KEY function that solves the problem
    """
...
async def background_refresh():
    """Background task to refresh all account data every 2 minutes"""
    while True:
        try:
...
async def startup_event():
    """Initialize MT5 and MongoDB on Bridge startup"""
...
async def health_check():
    ...
    return {
        ...
        "mt5": {
            ...
            "configured_accounts": len(ACCOUNT_CREDENTIALS)
        },
        "mongodb": {
    ...
    for account_number in ACCOUNT_CREDENTIALS.keys():
        if account_number in ACCOUNT_CACHE:
            ...
            accounts.append({
                ...
                "name": ACCOUNT_CREDENTIALS[account_number]['name'],
                "fund_type": ACCOUNT_CREDENTIALS[account_number]['fund_type'],
...
async def get_account_info(account_id: int):
    ...
    return {
        ...
        "name": ACCOUNT_CREDENTIALS[account_id]['name'],
        "fund_type": ACCOUNT_CREDENTIALS[account_id]['fund_type'],
        "provider": ACCOUNT_CREDENTIALS[account_id]['server'],
        "target_amount": 0,
        "live_data": live_data,
...
async def get_account_trades(account_id: int, limit: int = 100):
    ...
    try:
        # Login to the account to get its trades
