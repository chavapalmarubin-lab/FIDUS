...
BACKEND_URL = "https://analytics-hub-248.preview.emergentagent.com"
...
class TradingAnalyticsAPITester:
    ...
    def test_fund_endpoints(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Extract from fund object
                fund = data.get('fund', {})
                aum = fund.get('aum', 0)
                pnl = fund.get('total_pnl', 0)
                managers_count = fund.get('managers_count', 0)
    ...
    def print_test_summary(self):
        ...
        portfolio_status = "✅ HTTP 200" if any('Portfolio Total AUM' in r['test'] and r['success'] for r in self.test_results) else "❌ FAILED"
        balance_fund_status = "✅ HTTP 200" if any('BALANCE Fund AUM' in r['test'] and r['success'] for r in self.test_results) else "❌ FAILED"
        core_fund_status = "✅ HTTP 200" if any('CORE Fund AUM' in r['test'] and r['success'] for r in self.test_results) else "❌ FAILED"
        managers_ranking_status = "✅ HTTP 200" if any('Managers Total Count' in r['test'] and r['success'] for r in self.test_results) else "❌ FAILED"
        individual_manager_status = "✅ HTTP 200" if any('Individual Manager Name' in r['test'] and r['success'] for r in self.test_results) else "❌ FAILED"
        ...
        all_http_200 = all(r['success'] for r in self.test_results if 'Endpoint' in r['test'])
        ...
        fund_aggregations = any('BALANCE Fund AUM' in r['test'] and r['success'] for r in self.test_results) and any('CORE Fund AUM' in r['test'] and r['success'] for r in self.test_results)
        
        criteria_results.extend([
            f"{'✅' if all_http_200 else '❌'} All endpoints return HTTP 200",
