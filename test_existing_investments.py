...
# Authenticate
auth_response = session.post('https://auth-flow-debug-2.preview.emergentagent.com/api/auth/login', json={
    'username': 'admin',
...
if auth_response.status_code == 200:
    ...
    response = session.get('https://auth-flow-debug-2.preview.emergentagent.com/api/investments/client/client_003')
    if response.status_code == 200:
        ...
        for inv in investments:
            ...
            if inv_id:
                proj_response = session.get(f'https://auth-flow-debug-2.preview.emergentagent.com/api/investments/{inv_id}/projections')
