...
Last Updated: 2025-10-22 - Auto-Healing System Added
"""
...
async def one_time_setup(setup_key: str = None):
    ...
    if not setup_key or setup_key != EXPECTED_SETUP_KEY:
        logger.warning("[SECURITY] Invalid setup attempt")
    ...
    try:
        ...
        if not request_data or 'admin_token' not in request_data:
            raise HTTPException(status_code=400, detail="admin_token required")
        ...
        logger.warning("[SETUP] Adding ADMIN_SECRET_TOKEN to .env")
        ...
        if os.path.exists(env_path):
            with open(env_path, 'r') as f:
                ...
                if 'ADMIN_SECRET_TOKEN' in content:
                    token_exists = True
                    logger.warning("[SETUP] Token already exists")
