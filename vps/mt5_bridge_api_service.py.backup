...
async def shutdown_event():
    ...
    try:
        mt5.shutdown()
        logger.info("✅ MT5 Terminal shutdown complete")
    except Exception as e:
        ...
        if mt5.terminal_info() is not None:
            ...
            if password:
                try:
                    ...
                    logger.warning(f"⚠️ Could not get live data for {account_id}: {e}")
...
async def get_account_trades(account_id: int, limit: int = 100):
    ...
    try:
        ...
        logger.error(f"❌ Get account trades error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_accounts_summary():
    ...
    try:
        ...
        logger.error(f"❌ Get accounts summary error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
