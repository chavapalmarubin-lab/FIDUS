...
async def check_frontend_health() -> Dict[str, Any]:
    ...
    start = time.time()
    frontend_url = os.environ.get('REACT_APP_BACKEND_URL', 'https://risk-engine-v2.preview.emergentagent.com').replace('/api', '')
...
async def check_backend_health(db) -> Dict[str, Any]:
    ...
    try:
        ...
        backend_url = os.environ.get('REACT_APP_BACKEND_URL', 'https://risk-engine-v2.preview.emergentagent.com/api')
