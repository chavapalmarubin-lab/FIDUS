...
@router.get("/public/diagnostic-alejandro", tags=["Public"])
async def diagnostic_alejandro():
    """
    Comprehensive diagnostic to find Alejandro's data and provide fix recommendations
    """
    try:
        results = {
        ...
        print("🔍 Searching for Salvador Palma...")
        salvador = await db.salespeople.find_one({"referral_code": "SP-2025"})
        if salvador:
            results["salvador"] = {
                "found": True,
                ...
                "total_sales": float(salvador.get("total_sales_volume", 0)) if hasattr(salvador.get("total_sales_volume", 0), 'to_decimal') else float(salvador.get("total_sales_volume", 0)),
                "total_commissions": float(salvador.get("total_commissions_earned", 0)) if hasattr(salvador.get("total_commissions_earned", 0), 'to_decimal') else float(salvador.get("total_commissions_earned", 0))
            }
            print(f"✅ Found Salvador: {salvador['_id']}")
        else:
            results["salvador"] = {"found": False}
            results["recommendations"].append("❌ CRITICAL: Salvador Palma not found with code SP-2025")
            return results
        
        salvador_id = salvador["_id"]
        
        # 2. Search for Alejandro in USERS collection
        print("🔍 Searching for Alejandro in users...")
        ...
        if alejandro_user:
            results["alejandro_in_users"] = {
                "found": True,
                "id": str(alejandro_user["_id"]),
                "name": alejandro_user.get("name"),
                "email": alejandro_user.get("email"),
                "user_type": alejandro_user.get("user_type")
            }
            print(f"✅ Found Alejandro in users: {alejandro_user['_id']}")
        else:
            results["alejandro_in_users"] = {"found": False}
            print("⚠️  Alejandro not found in users collection")
        
        # 3. Search for Alejandro in CLIENTS collection
        print("🔍 Searching for Alejandro in clients...")
        ...
        if alejandro_client:
            results["alejandro_in_clients"] = {
                "found": True,
                "id": str(alejandro_client["_id"]),
                "name": alejandro_client.get("name"),
                "email": alejandro_client.get("email"),
                "user_id": str(alejandro_client.get("user_id")) if alejandro_client.get("user_id") else None,
                "referred_by": str(alejandro_client.get("referred_by")) if alejandro_client.get("referred_by") else None,
                "referred_by_name": alejandro_client.get("referred_by_name")
            }
            print(f"✅ Found Alejandro in clients: {alejandro_client['_id']}")
        else:
            results["alejandro_in_clients"] = {"found": False}
            print("⚠️  Alejandro not found in clients collection")
        
        # 4. Search for investments - try multiple ways
        print("🔍 Searching for Alejandro's investments...")
        investment_queries = []
        
        # Query 1: By client_id (if client exists)
        if alejandro_client:
            investment_queries.append({"client_id": str(alejandro_client["_id"])})
        
        # Query 2: By user_id (if user exists)
        if alejandro_user:
            investment_queries.append({"user_id": str(alejandro_user["_id"])})
        
        # Query 3: By name/email
        investment_queries.extend([
        ...
        for query in investment_queries:
            ...
            for inv in invs:
                inv_id = str(inv["_id"])
                if inv_id not in [i["id"] for i in all_investments]:
                    all_investments.append({
                        "id": inv_id,
                        "fund_type": inv.get("fund_type"),
                        "amount": float(inv.get("amount", 0)) if hasattr(inv.get("amount", 0), 'to_decimal') else float(inv.get("amount", 0)),
                        ...
                        "referred_by_name": inv.get("referred_by_name")
                    })
        
        results["investments"] = all_investments
        total_investment_amount = sum(inv["amount"] for inv in all_investments)
        print(f"✅ Found {len(all_investments)} investments totaling ${total_investment_amount:,.2f}")
        
        # 5. Check existing commissions for Salvador
        ...
        for comm in commissions:
            results["commissions"].append({
                "id": str(comm["_id"]),
                "amount": float(comm.get("amount", 0)) if hasattr(comm.get("amount", 0), 'to_decimal') else float(comm.get("amount", 0)),
                "source_type": comm.get("source_type"),
                "status": comm.get("status"),
                "payment_date": str(comm.get("payment_date"))
            })
        
        print(f"✅ Found {len(commissions)} existing commissions")
        
        # 6. Generate recommendations
        print("💡 Generating recommendations...")
        
        if not alejandro_client:
            if alejandro_user:
                results["recommendations"].append({
                    "priority": "HIGH",
                    "action": "CREATE_CLIENT_RECORD",
                    "details": f"Alejandro exists as user ({alejandro_user['_id']}) but has no client record",
                    "command": f"Create client with user_id: {alejandro_user['_id']}, referred_by: {salvador_id}"
                })
            else:
                results["recommendations"].append({
                    "priority": "CRITICAL",
                    "action": "CREATE_USER_AND_CLIENT",
                    "details": "Alejandro not found in users OR clients",
                    "command": "Need to create both user and client records"
                })
        
        if all_investments:
            not_linked_count = len([inv for inv in all_investments if inv["referred_by"] != str(salvador_id)])
            if not_linked_count > 0:
                results["recommendations"].append({
                    "priority": "HIGH",
                    "action": "LINK_INVESTMENTS_TO_SALVADOR",
                    "details": f"{not_linked_count} investments not linked to Salvador",
                    "command": f"Update {not_linked_count} investments with referred_by: {salvador_id}"
                })
            
            if alejandro_client:
                wrong_client_id = len([inv for inv in all_investments if inv["client_id"] != str(alejandro_client["_id"])])
                if wrong_client_id > 0:
                    results["recommendations"].append({
                        "priority": "HIGH",
                        "action": "FIX_CLIENT_ID_IN_INVESTMENTS",
                        "details": f"{wrong_client_id} investments have wrong/missing client_id",
                        "command": f"Update with client_id: {alejandro_client['_id']}"
                    })
        else:
            results["recommendations"].append({
                "priority": "CRITICAL",
                "action": "FIND_INVESTMENTS",
                "details": "No investments found for Alejandro by any search method",
                "command": "Need to manually locate the $18,151.41 CORE and $100,000 BALANCE investments"
            })
        
        if len(commissions) == 0 and all_investments:
            results["recommendations"].append({
                "priority": "MEDIUM",
                "action": "GENERATE_COMMISSIONS",
                "details": f"Found {len(all_investments)} investments but 0 commission records",
                "command": f"Generate commissions for ${total_investment_amount:,.2f} in investments"
            })
        
        # Summary
        results["summary"] = {
            "salvador_found": results["salvador"]["found"],
            "alejandro_in_users": results["alejandro_in_users"]["found"],
            "alejandro_in_clients": results["alejandro_in_clients"]["found"],
