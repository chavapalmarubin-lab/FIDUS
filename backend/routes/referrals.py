...
@router.post("/api/public/prospect-signup", tags=["Public"])
async def prospect_signup(
    ...
    try:
        ...
        if existing_lead:
            return {
                "success": True,
                "message": "¡Ya tenemos tu información! Alguien te contactará pronto.",
                "leadId": str(existing_lead.get("_id"))
            }
        ...
        new_lead = {
            ...
            "referred_by": str(salesperson["_id"]),
            "referral_code": referral_code,
            ...
            "crm_status_history": [{
                ...
                "changed_by": "system",
                "changed_by_name": "Referral Link Signup"
            }],
            "simulator_sessions": [],
            "last_contacted": None,
            "next_follow_up": None,
        ...
        # Update salesperson stats (if stats field exists)
        try:
            await db.salespeople.update_one(
                {"_id": salesperson["_id"]},
                {
                    ...
                    "$set": {
                        "last_lead_date": datetime.now(timezone.utc)
                    }
                }
            )
        except:
        ...
        logging.info(f"New prospect signup: {email} via referral code {referral_code}")
        
        return {
            "success": True,
            "message": "¡Gracias por tu interés! Te contactaremos en las próximas 24 horas.",
            "leadId": str(result.inserted_id),
            "agentName": salesperson.get("name")
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logging.error(f"Prospect signup error: {str(e)}")
        import traceback
        traceback.print_exc()
        raise HTTPException(
            status_code=500,
