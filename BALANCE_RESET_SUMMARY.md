...
- **URL**: `https://vps-bridge-fix.preview.emergentagent.com?skip_animation=true`
...
```bash
# Check zero balances
curl -s "https://vps-bridge-fix.preview.emergentagent.com/api/client/client_001/data" | jq '.monthly_statement'

# Check fund AUM
curl -s "https://vps-bridge-fix.preview.emergentagent.com/api/investments/funds/config" | jq '.funds[].aum'

# Check clients ready for investment  
curl -s "https://vps-bridge-fix.preview.emergentagent.com/api/clients/all" | jq '.ready_for_investment'
```
