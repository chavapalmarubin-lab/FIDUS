...
### Verify Backend
```bash
curl -X POST https://fidus-monitor.preview.emergentagent.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"admin","password":"password123","user_type":"admin"}'

# Use token from response
curl -X GET https://fidus-monitor.preview.emergentagent.com/api/admin/mt5/config/accounts \
